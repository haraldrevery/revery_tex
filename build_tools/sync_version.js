...
// Where the Legal page sends anyone exercising their AGPL right to the source.
const SOURCE_URL = 'https://github.com/haraldrevery/revery_tex';
const BUILD_INFO = 'www/jvscrpt_and_css_extra/build_info.js';
...
/**
 * The build stamp the Legal page reads.
 *
 * AGPL section 13 obliges a network deployment to offer the source *of what is
 * running*, so "there is a repo somewhere" is not enough — the running app has
 * to say which commit it is. Generated rather than committed-and-edited because
 * a stamp people update by hand is a stamp that lies.
 *
 * `--check` deliberately compares only the version. The commit legitimately
 * differs from HEAD between a build and the next commit, and failing CI on that
...
function commitHash() {
  try {
    ...
    // Building from a source tarball rather than a clone. The version and the
    // repo link still identify the source; only the exact commit is lost.
    return 'unknown';
  }
}

function buildInfo(v, commit) {
  return `// Generated by build_tools/sync_version.js — do not edit.
//
...
{
  const full = path.join(ROOT, BUILD_INFO);
  const existing = fs.existsSync(full) ? fs.readFileSync(full, 'utf8') : '';
  const found = /export const VERSION = "([^"]*)"/.exec(existing)?.[1];

  if (check) {
    if (found !== version) {
      console.error(`  ✗ ${BUILD_INFO} is ${found ?? 'missing'}, expected ${version}`);
      drift++;
    }
  } else {
    const next = buildInfo(version, commitHash());
    if (next !== existing) {
      fs.mkdirSync(path.dirname(full), { recursive: true });
      fs.writeFileSync(full, next);
      console.log(`  ✓ ${BUILD_INFO}  ${found ?? 'created'} → ${version}`);
      changed++;
    } else if (!explicit) {
      console.log(`  · ${BUILD_INFO} already ${version}`);
    }
  }
}
