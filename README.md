...
**The application as distributed is AGPL-3.0-or-later.** Not by preference — by
arithmetic. `texlyre_busytex.js` is a byte-identical copy of an AGPL-3.0-or-later
upstream and is pulled in by a plain ES `import` in `tex_engine_wasm.js`. That is
linking, not aggregation, so the combined work carries that licence. Calling it
Apache-2.0 while shipping it that way was simply false, and the declarations in
`package.json`, `tauri/Cargo.toml` and `tauri/tauri.conf.json` now say AGPL.

Files written by Harald Mark Thirslund are *additionally* available on their own
under **Apache-2.0** (`LICENSE-APACHE`). Apache-2.0 is one-way compatible into
AGPL-3.0, which is what makes the dual statement legitimate.

Four licence files, all of which ship in every installer:

| File | Covers |
|---|---|
| `LICENSE` | AGPL-3.0 — the work as distributed |
| `LICENSE-APACHE` | Apache-2.0 — Harald's own files, and pdf.js |
| `LICENSE-ASSETS` | the proprietary brand assets |
| `NOTICE` | attribution, component inventory, source offers |

The bundle is not homogeneous:
...
- **TeX Live binaries** (`busytex.wasm`) — pdfTeX, XeTeX and LuaTeX are **GPL-2+**.
  ...
  relationship TeXstudio has to a TeX installation. Aggregated, not combined —
  this is the argument that keeps the engines out of the licence calculation, and
  it is why the boundary in `tex_engine_wasm.js` matters legally and not only
  architecturally.
- **`texlyre-busytex`** wrapper — **AGPL-3.0-or-later**, and the reason for all of
  the above. `TexEngine` still exists partly so it can be replaced by an MIT
  wrapper over [`busytex/busytex`](https://github.com/busytex/busytex); doing so
  would let the whole application return to Apache-2.0.
- **pdf.js**, **CodeMirror**, **KaTeX** — Apache-2.0, MIT and MIT. Vendored
  ...
  the versions in `build_tools/`. Each licence file is vendored alongside.
- **Harald Revery fonts, logo and icons**, and **`www/image_assets/`** —
  proprietary, not covered by either code licence. See `LICENSE-ASSETS`, which
  grants every recipient permission to keep and use them in place to build, run,
  distribute and fork this software. That breadth is deliberate: anything
  narrower would act as a further restriction on the AGPL rights, which
  AGPL §7/§10 forbids. The carve-out is framed as declining trademark rights,
  which §7(e) permits.
- Do **not** copy Overleaf's `lezer-latex` grammar; it is AGPL-3.0. The editor
  ...
  (This predates the relicense and still stands: a second AGPL dependency would
  make the wrapper swap above no longer sufficient.)

### Obligations when releasing

- **AGPL §13 — the network clause.** A hosted copy must offer its corresponding
  source to everyone who loads it. The logo menu at the left of the toolbar is
  that offer — `Source code` is its first row — together with the Legal page
  (`legal.js`); `build_info.js`, generated by `sync_version.js`, is what makes it
  name the exact build. The logo sits at the far left because the toolbar clips
  from the right, and this is the one control that must survive every width.
- **The offer must not depend on a link being clickable.** Neither desktop shell
  opens a browser, by design, so `Source code` copies the address and the Legal
  page prints every URL as text with a Copy button beside it. Verified in the
  real shell by `test/run_electron.js`, not only in Chrome.
- **AGPL §6 — distribution.** All four licence files travel with the binaries,
  via `electron-builder.yml` `files:` and `tauri.conf.json` `resources`.
  `npm run verify:installers` asserts it.
- **The repository must be public.** Every source offer above resolves to it, and
  `vendor/texlyre-busytex/` is tracked there as the corresponding source.
